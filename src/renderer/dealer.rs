...
use ash::vk::{Buffer, BufferCreateInfo, BufferUsageFlags, Format, MemoryPropertyFlags, SharingMode, VertexInputAttributeDescription, VertexInputBindingDescription, VertexInputRate};
...
impl Dealer {
    pub fn new(instance: &Instance, device: &Device) -> Dealer {
        ...
        let allocator = unsafe { Allocator::new(create_info) }.expect("Failed to create allocator.");
        Dealer { allocator }
    }

    pub fn allocate_vertex_buffer(&self, device: &Device, vertices: &Vec<Vertex>) -> (Buffer, Allocation) {
